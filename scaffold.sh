...
npm install bull bull-board prom-client winston @supabase/supabase-js ioredis @types/ioredis
...
// Create an IoRedis client instead of the default Redis client
...
// Configure the task queue with proper typing
...
// Configure the async queue with the Redis URL directly
export const asyncQueue = new Queue('async-queue', process.env.REDIS_URL || 'redis://localhost:6379');
