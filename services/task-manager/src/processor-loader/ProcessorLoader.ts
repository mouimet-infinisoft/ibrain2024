...
export class ProcessorLoader {
  static loadProcessors(queueServer: TaskQueueServer, processorDir: string) {
    const processorFiles = fs.readdirSync(processorDir, {recursive:true, withFileTypes: true})
      .filter(file => file.name.endsWith('.processor.ts') || file.name.endsWith('.processor.js'));
        ...
        Object.values(processorModule).forEach(processorClass => {
          if (typeof processorClass === 'function') {
            ...
            if (queueName && action) {
              ...
              console.log(`Registered processor from ${file.name}`);
            }
          }
        });
      } catch (error) {
        console.error(`Error loading processor from ${file.name}:`, error);
      }
    });
  }
}
